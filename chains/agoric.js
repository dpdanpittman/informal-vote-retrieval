...
const AGORIC_API_ENDPOINT = 'https://main.api.agoric.net';

async function getProposalsInVotingAgoric() {
    try {
        const response = await axios.get(`${AGORIC_API_ENDPOINT}/cosmos/gov/v1beta1/proposals?proposal_status=2`);
...
async function getVoteStatusAgoric(proposalId, address) {
    ...
    try {
        const url = `${AGORIC_API_ENDPOINT}/cosmos/gov/v1beta1/proposals/${proposalId}/votes/${address}`;
...
async function getProposalStatusAgoric(proposalId) {
    try {
        const url = `${AGORIC_API_ENDPOINT}/cosmos/gov/v1beta1/proposals/${proposalId}`;
        const response = await axios.get(url);
...
async function getVotingDataAgoric(votingData) {
    const agoricAddress = 'agoric1x20lytyf6zkcrv5edpkfkn8sz578qg5s8qgulp'; // Replace with the actual address
...
module.exports = { getVotingDataAgoric };
