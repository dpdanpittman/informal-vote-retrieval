...
const COREUM_API_ENDPOINT = 'https://coreum-rest.publicnode.com';

async function getProposalsInVotingCoreum() {
    try {
        const response = await axios.get(`${COREUM_API_ENDPOINT}/cosmos/gov/v1beta1/proposals?proposal_status=2`);
...
async function getVoteStatusCoreum(proposalId, address) {
    ...
    try {
        const url = `${COREUM_API_ENDPOINT}/cosmos/gov/v1beta1/proposals/${proposalId}/votes/${address}`;
...
async function getProposalStatusCoreum(proposalId) {
    try {
        const url = `${COREUM_API_ENDPOINT}/cosmos/gov/v1beta1/proposals/${proposalId}`;
        const response = await axios.get(url);
...
async function getVotingDataCoreum(votingData) {
...
module.exports = { getVotingDataCoreum };
